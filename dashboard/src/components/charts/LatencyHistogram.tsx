...
export function LatencyHistogram({ records, width = 380, height = 180, target = 15 }: Props) {
  ...
  const markers: { label: string; value: number; color: string }[] = [
    { label: "p50", value: p50, color: chartColors.muted },
    { label: "p95", value: p95, color: chartColors.warning },
    { label: "p99", value: p99, color: chartColors.danger },
  ];
        ...
        <g transform={`translate(${MARGIN.left},${MARGIN.top})`}>
          {yScale.ticks(3).map((t) => (
            <g key={t}>
              ...
              <text x={-6} y={yScale(t) + 3} textAnchor="end" fontSize={9} fontFamily={TICK_FONT} fill={chartColors.faint}>{t}</text>
            </g>
          ))}

          {/* Target band (≤ target ms is the goal). */}
          <rect x={0} y={0} width={xScale(target)} height={innerH} fill={chartColors.accent} opacity={0.06} />
          ...
          {markers.map((m) => (
            <g key={m.label}>
              ...
              <text x={xScale(Math.min(m.value, xScale.domain()[1]!))} y={-8} textAnchor="middle" fontSize={8} fontFamily={TICK_FONT} fill={m.color}>{m.label}</text>
            </g>
          ))}

          <line x1={0} x2={innerW} y1={innerH} y2={innerH} stroke={chartColors.border} />
          {xScale.ticks(5).map((t) => (
            <text key={t} x={xScale(t)} y={innerH + 15} textAnchor="middle" fontSize={9} fontFamily={TICK_FONT} fill={chartColors.faint}>{t}ms</text>
          ))}
        </g>
      </svg>
      <div className="mt-1 flex justify-center gap-4 text-[11px]">
        {markers.map((m) => (
          <span key={m.label} className="flex items-center gap-1">
            ...
            <span className="font-code" style={{ color: m.color }}>{m.value.toFixed(1)}ms</span>
          </span>
        ))}
      </div>
    </div>
